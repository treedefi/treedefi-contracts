...
contract MasterChef is Ownable {
    ...
    constructor(
        ...
        devaddr = 0xb2F903e79d05600AC6BCD604e4Ac68a8717d1fD7; // 5% Treasury Wallet
        feeDonationAddress = 0x14f375Ba23F52a93CB768e80F0ECA123650C22D9; // 1/3 deposit fee to donation vault
        feeBuybackAddress = 0x32232a427A70f8C9019156c12Da9B3c392e07c1D; // 1/3 deposit fee to buyback 
        feeDevAddress = 0xdB67A848e237E4855b1BE722b16b7eD956a7210d; // 1/3 deposit fee to devs
        startTime = now;
    }
    ...
    function pendingTree(uint256 _pid, address _user) external view returns (uint256) {
        ...
        // Deflactionary System -2% minted token every week
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);

            uint256 calcTreePerBlock = 0;
            uint256 i = 0;
            uint256 calcTime = startTime;

            for (calcTime; calcTime < now; calcTreePerBlock++) {
                calcTime = calcTime + 1 weeks;
            }

            for (i = 0; i < calcTreePerBlock; i++) {
